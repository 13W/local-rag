...
/**
 * Patch existing memory points to add payload fields introduced in this version:
 * memory_type, access_count, updated_at, expires_at.
 * Uses setPayload — no re-embedding needed.
 * Only touches points that are actually missing one or more fields.
 */
export async function migrateMeta(
  ...
  for (const col of MEMORY_COLLECTIONS) {
    const memoryType = col.replace("memory_", "");
    let offset: string | number | undefined;
    ...
    while (true) {
      const result = await qd
        .scroll(col, {
          filter: { must: [{ key: "project_id", match: { value: projectId } }] },
          limit:        100,
          with_payload: ["memory_type", "access_count", "updated_at", "expires_at", "created_at"],
          with_vector:  false,
          ...(offset !== undefined && { offset }),
        })
        .catch((): { points: []; next_page_offset: undefined } => ({
      ...
      for (const pt of result.points) {
        ...
        if (!("memory_type"  in payload)) patch["memory_type"]  = memoryType;
        if (!("access_count" in payload)) patch["access_count"] = 0;
        if (!("updated_at"   in payload)) patch["updated_at"]   = payload["created_at"] ?? new Date().toISOString();
        if (!("expires_at"   in payload)) patch["expires_at"]   = "";

        if (Object.keys(patch).length > 0) {
          await qd.setPayload(col, { payload: patch, points: [String(pt.id)] });
          updated++;
        }
      }
